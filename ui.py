...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, parent=None):
        ...
        # Arc ring definitions: (radius_factor, arc_span, rotation_speed, thickness)
        self._arcs = [
            (0.44, 55, 1.8, 2.5),   # inner tight
            (0.44, 40, -2.4, 2.0),  # inner counter
            (0.52, 70, 1.2, 2.0),   # mid
            (0.52, 35, -1.6, 1.5),  # mid counter
            (0.60, 90, 0.9, 1.5),   # outer
            (0.60, 25, -1.4, 1.5),  # outer counter
            (0.60, 45, 2.2, 1.0),   # outer accent
            (0.68, 120, -0.6, 1.0), # outermost wide
            (0.68, 30, 1.0, 1.0),   # outermost accent
        ]
        self._arc_angles = [random.uniform(0, 360) for _ in self._arcs]
    ...
    def _step(self):
        ...
        for i, (_, _, spd, _) in enumerate(self._arcs):
            self._arc_angles[i] = (self._arc_angles[i] + spd * speak_mult * sleep_mult) % 360
    ...
    def paintEvent(self, _):
        ...
        # ─── 1. ROTATING ARC SEGMENTS ───
        p.setBrush(Qt.BrushStyle.NoBrush)
        for i, (rad_f, span, _, thick) in enumerate(self._arcs):
            radius = fw * rad_f * self._scale
            angle  = self._arc_angles[i]
            # Glow layers (outer glow first, then sharp core)
            glow_a = min(255, int(self._halo * 0.4))
            if is_sleeping: glow_a = int(glow_a * 0.35)
            pen_glow = QPen(alpha_clr(glow_a), thick + 4)
            pen_glow.setCapStyle(Qt.PenCapStyle.FlatCap)
            p.setPen(pen_glow)
            rect_g = QRectF(cx - radius, cy - radius, radius * 2, radius * 2)
            p.drawArc(rect_g, int(angle * 16), int(span * 16))

            # Sharp core line
            core_a = min(255, int(self._halo * 1.2))
            if is_sleeping: core_a = int(core_a * 0.4)
            pen_core = QPen(alpha_clr(core_a), thick)
            pen_core.setCapStyle(Qt.PenCapStyle.FlatCap)
            p.setPen(pen_core)
            p.drawArc(rect_g, int(angle * 16), int(span * 16))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._activity_log.setFixedHeight(140)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        lay.addWidget(_sec("COMMUNICATIONS LOG"))
        self._log = LogWidget()
        ...
        self._interrupt_btn = QPushButton("🛑  INTERRUPT JARVIS")
        ...
        self._interrupt_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background: #330000; border: 1px solid #ff0000; }}
...
class JarvisUI:
    ...
    def play_startup_sound(self):
        ...
        # Give the sound time to play before Jarvis starts talking
        # Most startup sounds are between 2-4 seconds.
        time.sleep(3.2)
